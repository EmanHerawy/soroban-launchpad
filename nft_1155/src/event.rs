...
pub(crate) fn transfer(e: &Env, from: Address, to: Address, category: i128, token_id: i128) {
    let topics = (symbol_short!("transfer"), from, to,category);
    e.events().publish(topics, token_id);
}

pub(crate) fn mint(e: &Env, admin: Address, to: Address , category: i128, token_id: i128) {
    let topics = (symbol_short!("mint"), admin, to,category);
    e.events().publish(topics,  token_id);
}
